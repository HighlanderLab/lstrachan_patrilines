...
if (trueData == FALSE) {
  name <- paste0("PatFatSis_", GEType, "_SNP", SNPArray, "_Route2")
  Step5_name <- paste0("Route2_", GEType, "_SNP", SNPArray)
  data_type <- paste0(GEType, "_", SNPArray)
  results_arg <- get(Step5_name)$phased_results_flipped
} else if (trueData == TRUE) {
  name <- "PatFatSis_SimTrue_Route2"
  Step5_name <- "Route2_SimTrue"
  data_type <- "True"
