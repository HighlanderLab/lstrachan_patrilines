...
create_excluded_siblings <- function(Known_Mothers) {
  ...
  maxLength <- max(sapply(result_list, length))
  result_list <- lapply(result_list, FUN = function(x) if (length(x) != maxLength) c(x, rep("", maxLength - length(x))) else (x))
...
create_excluded_mothers <- function(Known_Mothers) {
  ...
  # Convert the result_list to a data frame
  # Some lines would have more workers than others due to QC removed workers - pad with NA
  result_df <- do.call(rbind, result_list)
  result_df <- as.data.frame(result_df)
...
dir.create("Outputs/Colony", showWarnings = F)
#****** 1. SIMULATED *******************************************************
...
for (method in c("NoGE", "WithGE")) {
  for (n in c(5)) {
    ...
    Genotypes <- read.table(paste0("Data/Sim_", method, "/SNP_",n,"_", method, "_QC.ped"))
    ...
    pedigree_file <- pedigree_file[pedigree_file$id %in% Genotypes$V2, ]
    pedigree_file <- pedigree_file[pedigree_file$mother %in% Genotypes$V2, ]
    pedigree_file <- pedigree_file[pedigree_file$dpc %in% Genotypes$V2, ]
    ...
    print("Writing out the dat file")
    ...
    write.table(excluded_siblings,
                ...
                quote = FALSE)
    sink()
    setwd(paste0(repDir, "/Data/Colony"))
    system(paste0("cp ", softwareDir, "/colony/colony2s.ifort.out ."))
    cmd = paste0("./colony2s.ifort.out IFN:colony2_SNP", n, "_", method, ".dat")
    start = as.POSIXct(Sys.time())
    system(cmd)
    end = as.POSIXct(Sys.time())
    timing <- rbind(timing, c(method = method, n = n, Software = "Colony", Rep = Rep, 
    start = start, end = end, time = difftime(end, start, units = "mins")))
    colnames(timing) <- c("Method", "SNP_group", "Software", "Rep", "Start", "End", "Time")
    write.csv(timing, paste0(repDir, "/Outputs/Colony/Timing5.csv"), quote=F, row.names=F)
  }
}
...
# setwd(paste0(repDir, "/Data/Colony"))
# system(paste0("cp ", softwareDir, "/colony/* ."))
# system("ls *dat > Infiles_list.txt")
# system("mpirun -n 4 ./ColonyBatchRun_new.out  colony2s.ifort.out Infiles_list.txt")
